...
function Index({
  ...
  const { data: withdrawData } = useSWR(
    userInfo && userInfo?.platform_wallet && isWithdraw
      ? [withdraw.key, isWithdraw, inputVal]
      : null,
    () =>
      withdraw.fetcher({
        user_id: userInfo?.id,
        accessToken: userInfo?.access_token,
        withdrawable_amount: Number(inputVal),
      ...
      <Flex w="full" flexDir="column" justifyContent="space-between">
        ...
        >
          {loading ? (
            ...
            >
              {chooseType === "Deposit"
                ? `DEPOSIT WITH ${Number(inputVal).toFixed(2)} USDC`
                : `WITHDRAW ${Number(inputVal).toFixed(2)} USDC`}
            </Text>
          )}
        </Flex>
        {/* back */}
